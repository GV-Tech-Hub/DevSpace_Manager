...
class DeveloperWorkspace(QMainWindow):
    def __init__(self):
        ...
        # Calculate desired window size (85% of screen size and account for taskbar)
        width = int(screen.width() * 0.85)
        height = int(screen.height() * 0.85)
        
        # Calculate position to center the window and add padding for title bar
        left = int((screen.width() - width) / 2)
        top = int((screen.height() - height) / 2) + 30  # Added 30px padding for title bar
        ...
        self.setGeometry(left, top, width, height)  # Set size and position
        
        # Ensure window isn't maximized by default
        ...
        try:
            ...
            if os.path.isdir(path):
                ...
                if py_files:
                    ...
                    # Modified batch file creation to hide window
                    batch_path = os.path.join(path, "_temp_run.bat")
                    with open(batch_path, 'w') as f:
                        f.write('@echo off\n')
                        f.write('if "%PROCESSOR_ARCHITECTURE%"=="x86" pythonw "%~dp0%1"\n')
                        f.write('if "%PROCESSOR_ARCHITECTURE%"=="AMD64" pythonw "%~dp0%1"\n')
                        f.write('exit\n')
        ...
        try:
            ...
            if os.path.isdir(path):
                ...
                if py_files:
                    ...
                    # Modified command file creation to hide window
                    cmd_path = os.path.join(path, "_temp_run.cmd")
                    with open(cmd_path, 'w') as f:
                        f.write('@echo off\n')
                        f.write(f'pythonw -m pip install PyGithub PyQt5 > nul\n')
...
if __name__ == "__main__":
    hide_console()  # Hide console before creating the application
